...
// Minimal data structure for latest sample
struct IMU_Sample {
    uint32_t t_us; // micros() when sample captured
    float q[4];    // quaternion components
    uint8_t calib[3]; // calibration status from sensorValue.status
...
class IMU_BNO085 {
    ...
    IMU_BNO085(uint8_t csPin, uint8_t intPin, uint8_t resetPin = -1);
    ...
    // Change report at runtime
    bool setReport(sh2_SensorId_t report, uint32_t reportIntervalUs);

    // Read at most one sensor event; update latest if it matches the chosen report
    void tick()
    
    // True if at least one sample has been received recently
    bool ok() const { return _healthy; }

    // Access the most recent quaternion sample
    const IMU_Sample& latest() const { return _latest; }

    // get Euler angles in degrees from the latest quaternion
    IMU_EulerDeg latestEulerDegrees() const;

    // Diagnostics
    bool wasReset(); // checks chip-reset flag; re-enables report if needed
    uint32_t sequence() const; increasing sequence number
    uint8_t calibration() const; // 0..3 (3 = best)
    sh2_SensordId_t activeReport() const { return _reportType; }
    uint32_t activeIntervalUs() { return _reportIntervalUs; }
