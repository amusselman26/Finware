...
IMU_BNO085::IMU_BNO0855(uint8_t csPin, uint8_t intPin, int8_t resetPin)
...
void IMU_BNO085::quatToEuler(float qr, float qi, float qj, float qk, IMU_EulerDeg& outDeg) {
    ...
    float pitch = asinf(-2.0f * (qi*qk - qj*qr) / (sqr + sqi + sqj + sqk));
    float roll = atan2f(2.0f * (qr*qi + qj*qk), (sqr - sqi - sqj + sqk));
